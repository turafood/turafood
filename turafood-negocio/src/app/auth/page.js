...
export default function BizAuthPage() {
  ...
  const handleSignUp = async (e) => {
    ...
    if (error) {
      ...
      alert('¡Negocio registrado! Iniciando sesión...');
      router.push('/dashboard');
      router.refresh();
    }
  };
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100dvh', background: 'var(--bg)', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Background Decor */}
      <div style={{ position: 'absolute', top: '-10%', right: '-10%', width: '80%', height: '50%', background: 'linear-gradient(135deg, var(--primary) 0%, #ff7b5a 100%)', borderRadius: '50%', filter: 'blur(50px)', opacity: 0.15, zIndex: 0 }} />
      ...
      <div style={{ flex: 'none', padding: '60px 24px 20px', position: 'relative', zIndex: 1 }}>
        <h1 style={{ fontFamily: 'var(--font-bricolage)', fontWeight: 800, fontSize: '32px', lineHeight: 1.1 }}>
          TuraFood <br /><span style={{ color: 'var(--primary)' }}>Business</span>
        </h1>
      ...
      <div style={{ flex: 1, padding: '0 24px 40px', display: 'flex', flexDirection: 'column', position: 'relative', zIndex: 1, overflowY: 'auto' }}>
        ...
        {activeTab === 'login' ? (
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1 }}>
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '14px', padding: '0 16px', height: '56px' }}>
                ...
                <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="restaurante@ejemplo.com" style={{ flex: 1, border: 'none', background: 'transparent', outline: 'none', fontSize: '15px', color: 'var(--text)' }} />
              </div>
            </div>
          ...
          <form onSubmit={handleSignUp} style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1 }}>
            <div>
              <label style={{ display: 'block', fontSize: '13px', fontWeight: 700, color: 'var(--muted)', marginBottom: '8px' }}>Nombre del Negocio</label>
              <div style={{ display: 'flex', alignItems: 'center', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '14px', padding: '0 16px', height: '56px' }}>
                <span className="material-symbols-rounded" style={{ color: 'var(--muted)', marginRight: '12px', fontSize: '20px' }}>storefront</span>
                <input type="text" required value={businessName} onChange={(e) => setBusinessName(e.target.value)} placeholder="La Hamburguesería" style={{ flex: 1, border: 'none', background: 'transparent', outline: 'none', fontSize: '15px', color: 'var(--text)' }} />
              </div>
            </div>

            <div>
              <label style={{ display: 'block', fontSize: '13px', fontWeight: 700, color: 'var(--muted)', marginBottom: '8px' }}>Correo Electrónico (Admin)</label>
              <div style={{ display: 'flex', alignItems: 'center', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '14px', padding: '0 16px', height: '56px' }}>
                <span className="material-symbols-rounded" style={{ color: 'var(--muted)', marginRight: '12px', fontSize: '20px' }}>mail</span>
                <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@negocio.com" style={{ flex: 1, border: 'none', background: 'transparent', outline: 'none', fontSize: '15px', color: 'var(--text)' }} />
              </div>
            </div>
            ...
            <div style={{ marginTop: 'auto', paddingTop: '24px' }}>
              <button type="submit" disabled={loading} style={{ width: '100%', height: '56px', borderRadius: '999px', background: 'var(--primary)', color: '#fff', fontWeight: 700, fontSize: '16px', boxShadow: '0 10px 24px rgba(255,68,31,.32)', opacity: loading ? 0.7 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                {loading ? 'Creando cuenta...' : 'Crear Negocio'}
              </button>
            </div>
          </form>
        )}
